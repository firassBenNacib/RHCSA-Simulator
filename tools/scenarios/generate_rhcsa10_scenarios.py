...
def user_lab(n: int, user: str, group: str) -> dict[str, Any]:
    sid = f"lab-{n:02d}-users-groups-sudo"
    return lab(
        sid,
...
def flatpak_remote_lab(n: int, remote: str) -> dict[str, Any]:
    sid = f"lab-{n:02d}-flatpak-remote"
    return lab(
        sid,
...
def flatpak_package_lab(n: int, remote: str, app: str) -> dict[str, Any]:
    sid = f"lab-{n:02d}-flatpak-package"
    return lab(
        sid,
...
def exam_task_pool(seed: int) -> list[dict[str, Any]]:
    ...
    return [
        ...
        q("Create LVM mount", f"Create VG vg{letter}10 and LV data{letter} mounted at /mnt/data{letter}10.", "storage-lvm", [f"lvs /dev/vg{letter}10/data{letter} >/dev/null", f"findmnt -no TARGET /mnt/data{letter}10 | grep -qx /mnt/data{letter}10"], [["pvcreate /dev/sdb", f"vgcreate vg{letter}10 /dev/sdb", f"lvcreate -L 256M -n data{letter} vg{letter}10", f"mkfs.xfs -f /dev/vg{letter}10/data{letter}", f"mkdir -p /mnt/data{letter}10", f"echo '/dev/vg{letter}10/data{letter} /mnt/data{letter}10 xfs defaults 0 0' >> /etc/fstab", "mount -a"]]),
...
def make_exam(seed: int) -> dict[str, Any]:
    ...
    points = [5] * 12 + [4] * 10
    sid = f"mock-exam-{chr(ord('a') + seed)}"
