...
REPLAY_SSH_PUBLIC_KEY = "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIC4b7JPf0Pj8DNpzS5FCcfwpxi+9xSae4cCg9AckpTjP rhcsa-simulator-replay"
...
def install_replay_private_key(user: str) -> list[str]:
    return [
        ...
        *REPLAY_SSH_PRIVATE_KEY,
        "EOF",
        f"printf '%s\\n' '{REPLAY_SSH_PUBLIC_KEY}' > /home/{user}/.ssh/id_ed25519.pub",
...
def authorize_replay_key(user: str) -> list[str]:
    return [
        f"install -d -m 0700 -o {user} -g {user} /home/{user}/.ssh",
        f"printf '%s\\n' '{REPLAY_SSH_PUBLIC_KEY}' > /home/{user}/.ssh/authorized_keys",
