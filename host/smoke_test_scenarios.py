...
def scenario_requires_servervm(kind: str, scenario_id: str) -> bool:
    manifest_path = scenario_manifest_path(kind, scenario_id)
    if not manifest_path.exists():
        return False
    data = json.loads(manifest_path.read_text(encoding="utf-8"))
    return bool(data.get("flags", {}).get("requires_servervm", False))
...
def start_timeout_seconds(kind: str, scenario_id: str) -> int:
    if kind == "exam":
        return 900
    if scenario_requires_servervm(kind, scenario_id):
        return 720
    return 420
...
def main() -> int:
    ...
    for kind in iter_selected_kinds(args.kind):
        ids = filter_ids(scenario_ids(kind), args.start_from, args.end_at, args.only)
        print(f"\n== {kind}s ==")
        for index, scenario_id in enumerate(ids, start=1):
            print(f"[{index}/{len(ids)}] {scenario_id}")
            result = run_smoke(kind, scenario_id, args.start_timeout, args.check_timeout)
            if result.ok:
