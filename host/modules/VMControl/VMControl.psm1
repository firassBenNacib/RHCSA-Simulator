...
function Import-RhcsaOfflineIso {
    ...
    $major = $Matches[1]
    $destinationPath = Join-Path $ProjectRoot $sourceItem.Name
    ...
    elseif (Test-Path $destinationFullPath -PathType Leaf) {
        $destinationItem = Get-Item -LiteralPath $destinationFullPath
        if ($destinationItem.Length -ne $sourceItem.Length) {
            throw "Destination ISO already exists with a different size: $destinationFullPath"
        }
        $status = 'already-present'
    }
    else {
...
function Get-RhcsaPreflightStatus {
    ...
    $blockers = @()
    if (-not $isoDetected) {
        $blockers += "Missing ISO matching $expectedIsoPattern."
    }
