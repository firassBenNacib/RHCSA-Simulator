...
function Copy-RhcsaRepoCacheToServer {
    ...
    $prepareResult = Invoke-VagrantVmShellCommandCapture `
        -MachineName 'server' `
        -ProjectRoot $ProjectRoot `
        -Command "rm -rf $remoteStage && mkdir -p $remoteStage && chown vagrant:vagrant $remoteStage" `
        -RetryCount 1 `
        -RetryDelaySeconds 3 `
...
'@
    $verifyUploadResult = Invoke-VagrantVmShellCommandCapture `
        -MachineName 'server' `
        -ProjectRoot $ProjectRoot `
        -Command $verifyUploadCommand `
        -RetryCount 0 `
        -TimeoutSeconds 120 `
        -SkipConnectivityCheck `
        -SkipVagrantFallback
    if ([int]$verifyUploadResult.ExitCode -ne 0) {
        Write-FailureTranscript -StdOut $verifyUploadResult.StdOut -StdErr $verifyUploadResult.StdErr | Out-Null
        throw 'Repo cache upload did not create BaseOS and AppStream metadata on server.'
    }
...
sudo chown root:root /var/www/html/repo
sudo find /var/www/html/repo -type d -exec chmod 755 {} +
sudo find /var/www/html/repo -type f ! -perm -004 -exec chmod a+r {} +
test -x /var/www/html/repo/BaseOS/repodata
test -r /var/www/html/repo/BaseOS/repodata/repomd.xml
test -x /var/www/html/repo/AppStream/repodata
test -r /var/www/html/repo/AppStream/repodata/repomd.xml
...
'@
    ...
    if ([int]$installResult.ExitCode -ne 0) {
        ...
        $failureTail = ((@($installResult.StdErr) + @($installResult.StdOut)) | Where-Object { -not [string]::IsNullOrWhiteSpace([string]$_) } | Select-Object -Last 8) -join ' | '
        if ([string]::IsNullOrWhiteSpace($failureTail)) {
            throw 'Failed to install the uploaded repo cache on server.'
        }
        throw "Failed to install the uploaded repo cache on server. Last output: $failureTail"
    }
...
function Invoke-BaselineGuestProvisioning {
    ...
    $isCacheOnlyServerProvisioning = (
        $MachineName -eq 'server' -and
        [string]::IsNullOrWhiteSpace((Get-RhcsaOfflineIsoPath -ProjectRoot $ProjectRoot -Profile $profile)) -and
        (Test-RhcsaRepoCacheReady -ProjectRoot $ProjectRoot -Profile $profile)
    )
    ...
    try {
        ...
        if ($isCacheOnlyServerProvisioning) {
            throw "Direct cache-backed guest provisioning failed for ${MachineName}: $fallbackReason"
        }
...
function Get-BaselineStatus {
    ...
    $baseSnapshotState = Get-BaseSnapshotState -ProjectRoot $ProjectRoot
    $liveCleanState = $null
    $liveCleanStatePath = Get-LiveCleanBaselineStatePath -ProjectRoot $ProjectRoot
    if (Test-Path -LiteralPath $liveCleanStatePath -PathType Leaf) {
        try {
            $liveCleanState = (Get-Content -LiteralPath $liveCleanStatePath -Raw) | ConvertFrom-Json
        }
        catch {
            $liveCleanState = $null
        }
    ...
    function Test-MachineRecordedForCurrentProfile {
        param(
            [AllowNull()]
            [object]$State,
            [string]$MachineName,
        ...
        $profileProperty = $State.PSObject.Properties['profile']
        $snapshotNameProperty = $State.PSObject.Properties['snapshot_name']
        if (
            $null -eq $profileProperty -or
            $null -eq $snapshotNameProperty -or
            [string]$profileProperty.Value -ne $projectProfile -or
            [string]$snapshotNameProperty.Value -ne 'base-clean'
        ) {
            return $false
        }

        $machineState = $State.machines.PSObject.Properties[$MachineName]
        return ($null -ne $machineState -and [string]$machineState.Value.vm_id -eq $VmId)
    }
    ...
    foreach ($machineName in $machineNames) {
        ...
        $machineVmId = ((Get-Content -LiteralPath $idFile -Raw) -as [string]).Trim()
        $profileRecordedForMachine = (
            (Test-MachineRecordedForCurrentProfile -State $baseSnapshotState -MachineName $machineName -VmId $machineVmId) -or
            (Test-MachineRecordedForCurrentProfile -State $liveCleanState -MachineName $machineName -VmId $machineVmId)
        )
        if (
            ([string]::IsNullOrWhiteSpace($actualBoxName) -and -not $profileRecordedForMachine) -or
            (-not [string]::IsNullOrWhiteSpace($actualBoxName) -and [string]$actualBoxName -ne [string]$expectedBoxName)
        ) {
