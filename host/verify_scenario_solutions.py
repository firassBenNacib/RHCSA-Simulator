...
def resolve_exam_check_clause(clause: str) -> tuple[str, str]:
    ...
    for _index, (_start, end, token) in enumerate(token_spans[1:], start=1):
        ...
        if not remote_command:
            return "clientvm", clause.strip()
        return "servervm", f"{negate_prefix}{remote_command}".strip()
...
def default_vm_for_task(manifest: dict, task_text: str) -> str:
    ...
    if lowered.startswith("on servervm"):
        return "servervm"
    if lowered.startswith("on clientvm"):
        return "clientvm"
    if re.search(r"\bon clientvm\b", lowered):
        return "clientvm"
    if re.search(r"\bon servervm\b", lowered):
        return "servervm"
    if "servervm" in lowered and "clientvm" not in lowered:
        return "servervm"
    if "clientvm" in lowered and "servervm" not in lowered:
        return "clientvm"
    if "servervm" in lowered and "clientvm" in lowered:
        return "clientvm"
    if "servervm" in description_lower and "clientvm" not in description_lower:
        return "servervm"
    if "clientvm" in description_lower and "servervm" not in description_lower:
        return "clientvm"
    vm_scripts = manifest.get("vm_scripts", {})
    if len(vm_scripts) == 1:
        return next(iter(vm_scripts))
    if manifest.get("flags", {}).get("requires_servervm", False) and "clientvm" not in description_lower:
        return "servervm"
    return "clientvm"
...
def resolve_vm_alias(hostname: str) -> str | None:
    if hostname in {"servervm", "192.168.122.3"}:
        return "servervm"
    if hostname in {"clientvm", "192.168.122.2"}:
        return "clientvm"
    return None
...
def translate_task(
    ...
    *,
    ...
    # Each task should begin on the VM implied by its own wording/manifest.
    # Carrying the previous task's VM forward breaks mixed-host labs like lab 11.
    ...
    while i < len(normalized_lines):
        ...
        if line.startswith("#"):
            lowered = line.lower()
            if "run on servervm" in lowered or lowered.startswith("# on servervm"):
                flush(f"task {task_index + 1}")
                current_vm = "servervm"
            elif "run on clientvm" in lowered or lowered.startswith("# on clientvm"):
                flush(f"task {task_index + 1}")
                current_vm = "clientvm"
            i += 1
            continue
...
def start_or_reset_scenario(scenario_id: str, mode: str, start_timeout: int) -> tuple[bool, str]:
    attempts = 0
    output = ""
    transient_markers = (
    ...
    while attempts < 3:
        attempts += 1
        try:
            start_proc = run_ps("start", "-Id", scenario_id, "-Mode", mode, timeout_seconds=start_timeout)
        except subprocess.TimeoutExpired:
        ...
        if start_proc.returncode == 0:
            ...
            try:
                ...
                if destroy_proc.returncode == 0:
                    ...
                    output += up_proc.stdout + up_proc.stderr
                    if up_proc.returncode == 0:
                        retry_start_proc = run_ps("start", "-Id", scenario_id, "-Mode", mode, timeout_seconds=start_timeout)
...
def wait_for_started_scenario(manifest: dict) -> None:
    if manifest["flags"].get("requires_servervm", False):
        wait_for_vm_ssh("servervm", timeout=180)
    wait_for_vm_ssh("clientvm", timeout=180)

    delay = 2
    objective_tags = set(manifest.get("objective_tags", []))
    if "containers" in objective_tags:
        delay = 15
    elif manifest["flags"].get("requires_servervm", False):
        delay = 5
...
def verify_lab(scenario_id: str, manifest: dict, exec_timeout: int, start_timeout: int) -> tuple[bool, list[str]]:
    messages: list[str] = []
    ...
    started, start_output = start_or_reset_scenario(scenario_id, "Lab", start_timeout)
    if not started:
    ...
    for index in range(task_count):
        ...
        if "No active lab run found." in check_output:
            restarted, restart_output = start_or_reset_scenario(scenario_id, "Lab", start_timeout)
            if not restarted:
...
def verify_exam(scenario_id: str, manifest: dict, exec_timeout: int, start_timeout: int) -> tuple[bool, list[str]]:
    messages: list[str] = []
    started, start_output = start_or_reset_scenario(scenario_id, "Exam", start_timeout)
    if not started:
        return False, [start_output]
    try:
        wait_for_started_scenario(manifest)
...
def main() -> int:
    ...
    for kind in kinds:
        ...
        for index, scenario_id in enumerate(ids, start=1):
            ...
            manifest = load_manifest(kind, resolve_scenario_token(scenario_ids(kind), scenario_id))
            if args.audit_only:
                ok, messages = audit_scenario(kind, scenario_id, manifest)
            elif kind == "lab":
                ok, messages = verify_lab(scenario_id, manifest, args.exec_timeout, args.start_timeout)
            else:
                ok, messages = verify_exam(scenario_id, manifest, args.exec_timeout, args.start_timeout)
            if ok:
                print("  ok")
