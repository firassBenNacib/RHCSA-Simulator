...
## Task 01 - Create /usr/local/sbin/rhcsa10-timer.sh so it appends TIMER OK to /var/l (server) - 10 pts

On server, create /usr/local/sbin/rhcsa10-timer.sh so it appends TIMER OK to /var/log/rhcsa10-timer.log.

---

## Task 02 - Create a oneshot service named rhcsa10-timer.service that runs /usr/loca (server) - 10 pts

On server, create a oneshot service named rhcsa10-timer.service that runs /usr/local/sbin/rhcsa10-timer.sh.

---

## Task 03 - Create rhcsa10-timer.timer so it runs every 5 minutes, is persistent, an (server) - 10 pts
