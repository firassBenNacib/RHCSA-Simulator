...
Configure a repository file on client with BaseOS and AppStream served from server, enabled, and with gpgcheck disabled.
...
Configure Apache on client so it serves the existing site on TCP port 8181.
...
Configure client to boot into multi-user.target by default. Ensure rsyslog is enabled and running. If postfix is installed, disable it and stop it.

---

## Question 20 - Install And Remove Packages (client) - 4 pts

Use the prepared local repositories to install the packages tree and dos2unix on client. Remove dos2unix and leave tree installed.
