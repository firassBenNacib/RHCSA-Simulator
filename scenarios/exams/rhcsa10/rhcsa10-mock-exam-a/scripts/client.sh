...
rhcsa_remove_matching_lines 'servera.exam10.lab' /etc/hosts
connection_name="$(rhcsa_get_lab_connection_name || true)"
rhcsa_reset_lab_ipv4_profile "$connection_name"

# --- Reset repos ---
rhcsa_reset_repo_directory /root/.repo-backup-exam-a

# --- Remove flatpak remotes ---
flatpak remote-delete --system examaflatpak >/dev/null 2>&1 || true
flatpak uninstall --system -y org.rhcsa.Tools >/dev/null 2>&1 || true

# --- Remove users, groups, sudoers ---
userdel -r usera10 >/dev/null 2>&1 || true
groupdel teama10 >/dev/null 2>&1 || true
rm -f /etc/sudoers.d/teama10-systemctl

# --- SELinux: reset boolean, remove port labels ---
setsebool httpd_can_network_connect 0 2>/dev/null || true
semanage port -d -t http_port_t -p tcp 8100 >/dev/null 2>&1 || true

# --- Firewalld: remove port, reset service ---
firewall-cmd --permanent --remove-port=8100/tcp >/dev/null 2>&1 || true
...
chcon -t user_tmp_t /var/www/html/a.html 2>/dev/null || true
...
rm -f /etc/systemd/system/examatimer.service /etc/systemd/system/examatimer.timer
rm -f /usr/local/sbin/examatimer.sh
systemctl disable --now examatimer.timer >/dev/null 2>&1 || true

# --- Cron: remove crontab ---
crontab -r -u usera10 >/dev/null 2>&1 || true

# --- Autofs: remove configs ---
systemctl disable --now autofs >/dev/null 2>&1 || true
rm -f /etc/auto.remotea /etc/auto.master.d/a.autofs
automount -u >/dev/null 2>&1 || true
rm -rf /remotea

# --- LVM: wipe /dev/sdb ---
umount /mnt/dataa10 >/dev/null 2>&1 || true
sed -i '\#/mnt/dataa10#d' /etc/fstab
lvremove -fy /dev/vga10/dataa >/dev/null 2>&1 || true
vgremove -fy vga10 >/dev/null 2>&1 || true
...
rm -f /usr/local/bin/a-who /root/a-shell-users.txt /root/a-etc.tar.gz
rm -f /root/a-original /root/a-hard /root/a-soft
