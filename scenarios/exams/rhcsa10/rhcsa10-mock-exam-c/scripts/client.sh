...
semanage port -d -t http_port_t -p tcp 8102 >/dev/null 2>&1 || true

# --- Firewalld: remove port, reset service ---
firewall-cmd --permanent --remove-port=8102/tcp >/dev/null 2>&1 || true
...
chcon -t user_tmp_t /var/www/html/c.html 2>/dev/null || true
...
rm -f /etc/systemd/system/examctimer.service /etc/systemd/system/examctimer.timer
rm -f /usr/local/sbin/examctimer.sh
systemctl disable --now examctimer.timer >/dev/null 2>&1 || true

# --- Cron: remove crontab ---
crontab -r -u userc10 >/dev/null 2>&1 || true

# --- Autofs: remove configs ---
systemctl disable --now autofs >/dev/null 2>&1 || true
rm -f /etc/auto.remotec /etc/auto.master.d/c.autofs
automount -u >/dev/null 2>&1 || true
rm -rf /remotec

# --- LVM: wipe /dev/sdb ---
umount /mnt/datac10 >/dev/null 2>&1 || true
sed -i '\#/mnt/datac10#d' /etc/fstab
lvremove -fy /dev/vgc10/datac >/dev/null 2>&1 || true
vgremove -fy vgc10 >/dev/null 2>&1 || true
...
rm -f /usr/local/bin/c-who /root/c-shell-users.txt /root/c-etc.tar.gz
rm -f /root/c-original /root/c-hard /root/c-soft
